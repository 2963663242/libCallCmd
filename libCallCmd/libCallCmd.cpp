// libCallCmd.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//

#include <iostream>
#include "process.hpp"
int main()
{
   // Process process;
   // //thread thread1([&]() {
   // //    
   // //    process.open(".\\winBin\\tubepaw.exe ", [](const char* bytes) {
   // //        cout << "Output from stdout: " << string(bytes) << endl;
   // //        });
   // //    });
   // //thread1.detach();
   // 
   // //getchar();
   // /*process.aysnOpen(".\\winBin\\tubepaw.exe ", [](const char* bytes) {
   //             cout << "Output from stdout: " << string(bytes) << endl;
   //             });*/
   // process.kill();
   // process.get_exit_status();
   ///* thread thread2([&]() {

   //     process.open(".\\winBin\\tubepaw.exe ", [](const char* bytes) {
   //         cout << "Output from stdout: " << string(bytes) << endl;
   //         });
   //     });
   // thread2.detach();*/
   //// process.kill();
   // process.aysnOpen(".\\winBin\\tubepaw.exe ", [](const char* bytes) {
   //     cout << "Output from stdout: " << string(bytes) << endl;
   //     });
   // getchar();
   // process.kill();
   // process.get_exit_status();
    const int n = 10;
    Process proceses[n];
  
    for (int i = 0; i < n;i++) {
        proceses[i].aysnOpen(".\\winBin\\tubepaw.exe ", [](const char* bytes) {
            cout << "Output from stdout: " << string(bytes) << endl;
            });
    }
    std::this_thread::sleep_for(std::chrono::seconds(1));
    for (int i = 0; i < n; i++) {
        proceses[i].kill();
        proceses[i].get_exit_status();
    }
    return 0;
}
